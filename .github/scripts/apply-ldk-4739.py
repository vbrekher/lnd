...
manager = root / "lightning/src/ln/channelmanager.rs"
text = manager.read_text()
old = "\t\t\t\t\t\t\t\t\t\tdebug_assert!(found_blocker);\n"
assert old in text, "target debug assertion not found"
text = text.replace(
    old,
...
tests = root / "lightning/src/ln/chanmon_update_fail_tests.rs"
text = tests.read_text()
old_import = "use crate::routing::router::{PaymentParameters, RouteParameters};"
new_import = "use crate::routing::router::{Path, PaymentParameters, Route, RouteHop, RouteParameters};"
assert old_import in text, "routing import anchor not found"
...
fn test_duplicate_onchain_forward_claim_with_other_downstream_blocker() {
	...
	nodes[0]
		.node
		.send_payment_with_route(route, mpp_hash, onion, PaymentId(mpp_hash.0))
		.unwrap();
	...
	while mpp_parts_received < mpp_parts.len() {
		...
		nodes[1]
			.node
			.handle_commitment_signed_batch_test(node_a_id, &mpp_event.commitment_msg);
	...
	nodes[1]
		.node
		.handle_update_fulfill_htlc(node_a_id, as_updates.update_fulfill_htlcs[0].clone());
	check_added_monitors(&nodes[1], 1);
...
'''
assert "fn test_duplicate_onchain_forward_claim_with_other_downstream_blocker()" not in text
